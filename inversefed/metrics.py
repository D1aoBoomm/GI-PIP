...
def anomaly_loss(x, model, mean_std, dataset):
    dm, ds = mean_std
    
    # imagenet AE训练没有归一化，要反归一化
    if(dataset=='ImageNet'):
        x = torch.clamp(x * ds + dm, 0, 1)
        ano = torch.nn.functional.mse_loss(x, model(x))
        ano = ano - 0.005
    
    # cifar10 AE训练没有归一化，要反归一化
